...
#### 2.1.1 Example: Sample from a complex distribution.

We would like to sample from a distribution which has this formula $p(x) =\frac{x^3 -6x^2 + 4x + 20}{84}$ which is defined on $[0,6]$. 
let's plot it.

```{r}
curve((x^3 - 6*x^2 + 4*x + 20)/84,from=0,to=6,xlab = 'density')
grid()
```

1. The target distribution is known to us $p(x)$.
2. Proposal distribution: choose something which is easy to sample from and is symmetric. Let's use a normal with standard deviation of 0.5. Our proposal distribution which we use to generate random samples from, based on previous samples is going to be $q(x^*|x_{t-1}) = \text{Normal}(\text{mean}=x_{t-1},\text{sd}=0.5)$.
3. Choose a starting value $x_0 = 5$ and run the chain for 10000 iterations.

Here are the mechanics of **Metropolis Algorithm** and the code in R.

* Use previous sample and proposal distribution to generate the random proposal $x^\star$. I.e. Generate a random sample from Normal$(x_0,0.5)$ = Normal$(5,0.5)$, in the following R code we get. $x^\star=5.277163$.
...
* calculate the ratio of $\frac{p(x^\star)}{p(x_0)} = \frac{p(5.277163)}{p(5)}= \frac{5.277163^3 -6 (5.277163)^2 + 4(5.277163) + 20}{5^3 -6(5)^2 + 4(5) + 20}=1.398584$

```{r}
t_ratio = (5.277163^3 -6 *(5.277163)^2 + 4*(5.277163) + 20) / (5^3 -6*(5)^2 + 4*(5) + 20)
t_ratio
```

* The value of $u=0.3896344$ is less than 1.3985, so accept $x^\star = 5.277163$ as a new sample from this distribution. i.e. $s_1 = 5.277163$.

* if the value of $u \ge \frac{p(x^\star)}{p(x_0)}$ then we reject $x^\star$ as a sample and set $s_1 = 5$.
...
curve(p(x), col=2, from=0, to=6, n=100, lwd=3,frame.plot = FALSE)

# decide on a proposal distribution: normal with SD=0.5
q <- function(x,std_dev) {
  rnorm(n = 1, mean = x, sd = std_dev)
}
...
for (i in 2:nStep) {
  ...
  x_star = rnorm(1,mean = xs[i-1], sd = 0.5)
  
  # draw a random sample from uniform(0,1) distribution.
  u = runif(n=1)
  
  # calculate the ratio of the proposed sample to the previous sample
  alpha = ifelse(x_star >= 0 & x_star <= 6, x_star^3 -6*x_star^2 + 4*x_star + 20,0) / (xs[i-1]^3 -6*xs[i-1]^2 + 4*xs[i-1] + 20)
...
# Run the metropolis algorithm using functions.
for (i in 2:nStep) {
  # draw a sample from proposal distribution using previous sample.
  x_star = q(xs[i-1],std_dev = 1.2)
...
plot(xs[1:1000], type="s", xpd=NA, ylab="Parameter", xlab="Sample", las=1)
points(xs[1:1000],pch = 1, col = 4)
usr <- par("usr")
...
hist(xs, 50, freq=FALSE, main="", ylim=c(0, .5), las=1,
     xlab="x", ylab="Probability density")
z <- integrate(p, 0, 6)$value
...
res.fast <- run(1, p, function(x) rnorm(1, mean = x,  sd = 5), 10000)

# small moves: A small variance means the proposed values are not that different from the current value.
res.slow <- run(1, p, function(x) rnorm(1, mean = x,  sd = .1), 10000)
...
plot(xs[1:100], type="s", xpd=NA, ylab="Parameter", xlab="Sample", las=1,
     col="grey",ylim=c(0,6))
points(xs[1:100],pch =1,col = 'grey')
# plot the new chains with the big and small sd in the proposal distribution.
lines(res.fast[1:100], col='red')
points(res.fast[1:100],pch =1,col = 'red')
lines(res.slow[1:100], col='green')
points(res.slow[1:100],pch =1,col = 'green')
usr <- par("usr")
xx <- seq(usr[3], usr[4], length=301)
...
hist(xs, 50, freq=FALSE, ylim=c(0, .5), las=1,
     xlab="x", ylab="Probability density",main = 'proposal with SD = 0.5')
curve(p(x) / z, add=TRUE, col="red", n=200)

hist(res.fast, 50, freq=FALSE, ylim=c(0, .5), las=1,
     xlab="x", ylab="Probability density",main = 'proposal with SD = 3')
curve(p(x) / z, add=TRUE, col="red", n=200)

hist(res.slow, 50, freq=FALSE, ylim=c(0, .5),xlim=c(0,6), las=1,
     xlab="x", ylab="Probability density",main = 'proposal with SD = 0.1')
curve(p(x) / z, add=TRUE, col="red", n=200)
...
q <- function(x){
  ifelse(x >=0 & x <= 6, rnorm(1,mean = x, sd = 1.3),0)
}


res.long <- run(2, p,  q, 500000)
# make plots.
hist(res.long, 100, freq=FALSE, main="", ylim=c(0, .5), las=1,
...
hist(res.long.burn.thin, 100, freq=FALSE, main="", ylim=c(0, .5), las=1,
     xlab="x", ylab="Probability density", col="grey")
z <- integrate(p, -Inf, Inf)$value
curve(p(x) / z, add=TRUE, col="red", n=200)
...
final_res = lambdas[seq(1000,length(lambdas))]
length(final_res)
...
The mean is 48.7 and the 90% percentile interval is (45.2, 52.1).
